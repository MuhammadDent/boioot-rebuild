...
- **Core Features:**
    ...
    - **Booking MVP:** Additive booking flow for DailyRent properties only. `Properties.IsBookable` gates the UI and API, `POST /api/bookings` is authenticated, blocks self-booking and non-future/invalid ranges, and stores pending booking requests in the `Bookings` table. Booking rows persist `PricePerNight`, `TotalAmount`, `CommissionPercent`, `CommissionAmount`, and `PaymentStatus`; totals are calculated on create from `Property.Price × nights`, and commission uses `Booking:CommissionPercent` config with a 10% default. PostgreSQL/SQLite schema patches are idempotent and run through startup schema evolution, not manual migrations. `Approved` is the canonical accepted booking status; legacy `Confirmed` rows remain readable but are normalized to `Approved` in backend responses, availability/conflict checks, and frontend display. New acceptance flows write `Approved` only.
