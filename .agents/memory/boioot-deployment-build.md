...
- Prod DB bootstrap edge: `__EFMigrationsHistory` can exist EMPTY in prod (publish-time schema copy carries structure, not rows) → MigrateAsync replays InitialSchema → 42P07 → whole init (patches+seeding) silently aborts (Program.cs swallows it, app "runs without DB"). DatabaseStartupService now backfills migration IDs when history-table-exists-but-empty + Users exists.
