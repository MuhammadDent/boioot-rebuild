...
- "Login works but register 500s" after a schema recovery is a symptom trap: login only reads Users, while register writes Users + (for commercial roles) Accounts/AccountUsers/Subscriptions/SubscriptionHistories/Companies/Agents and allocates a ReferenceGenerator sequence. Login succeeding never proves register works. In practice the empty-history recovery (baseline + MigrateAsync + patches) fixed register too — no register-specific schema fix was needed; verify with a live register before assuming a new bug.
- DB isolation (verified 2026-07-02): boioot.com runs on Vercel (Cloudflare front) with its OWN database — fully separate infra from the Replit deployment (Google Frontend, Replit-managed Neon db `neondb`, user `neondb_owner`). Dev workspace uses helium `heliumdb`. Nothing in this workspace holds boioot.com DB credentials; migrations/seeders here can only ever touch the helium dev DB or the Replit-managed prod DB. The user-set `DATABASE_URL` secret is a broken template (literal `HOST:PORT` placeholders) — backend falls back to PG* vars; never parse it directly.
