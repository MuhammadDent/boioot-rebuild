...
// initialHeroImage: fetched server-side by page.tsx → stable URL, no flicker.
// ContentContext still provides text content (title, subtitle, CTA) after hydration.
...
export default function HomePageClient({ initialHeroImage }: { initialHeroImage: string }) {
  ...
  // ── Hero image: server-provided — stable on first paint, no hydration flicker ──
  ...
  const updateScrollBtns = useCallback(() => {
    ...
    setCanScrollPrev(sl < 0);
    setCanScrollNext(Math.abs(sl) < overflow);
  }, []);
