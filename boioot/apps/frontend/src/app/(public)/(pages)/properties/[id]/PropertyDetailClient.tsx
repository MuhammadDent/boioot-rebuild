...
function PhoneRevealButton({ phone }: { phone: string }) {
  ...
  return (
    <button
      type="button"
      onClick={() => { setRevealed(true); trackEvent("click_phone"); }}
      style={sharedStyle}
    >
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 5.61 5.61l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
...
export default function PropertyDetailClient({ property }: { property: PropertyResponse }) {
  ...
  const openBooking = useCallback(() => {
    setBookingError("");
    trackEvent("booking_start", { property_id: id });
    if (!user) { openAuthModal(() => setBookingOpen(true)); return; }
    setBookingOpen(true);
  }, [user, openAuthModal, id]);
  ...
  const submitBooking = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setBookingError("");
    trackEvent("booking_submit", { property_id: id, nights: getNightCount(bookingForm.startDate, bookingForm.endDate) });
    try {
      ...
      trackEvent("booking_success", { property_id: id, nights: getNightCount(bookingForm.startDate, bookingForm.endDate) });
      ...
      trackEvent("booking_failed", { property_id: id });
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  ...
                  {property.ownerPhone && property.ownerHasWhatsappContact !== false && (
                    <a
                      ...
                      onClick={() => trackEvent("click_whatsapp", { property_id: property.id })}
                    >
