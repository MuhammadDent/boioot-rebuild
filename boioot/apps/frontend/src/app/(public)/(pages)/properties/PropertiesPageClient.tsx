...
// ─── Client component (needs Suspense because it uses useSearchParams) ────────
...
export default function PropertiesPageClient({
  ...
  // ── State initialised from server-provided initial data ───────────────────
  ...
  const [hasPrev, setHasPrev]         = useState(initialHasPrev);
  const [retryKey, setRetryKey]       = useState(0);

  // Skip the very first useEffect run — initial data already came from the server.
  ...
  useEffect(() => {
    // First render: initial data came from the server — skip client fetch.
