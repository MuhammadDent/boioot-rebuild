...
import { adminApi, type UpsertOwnershipTypePayload } from "@/features/admin/api";
...
export default function AdminOwnershipTypesPage() {
  ...
  const [editTarget, setEditTarget] = useState<OwnershipTypeConfig | null>(null);
  const [saving, setSaving]     = useState(false);
  const [deleting, setDeleting] = useState<string | null>(null);

  const load = useCallback(async () => {
    setFetching(true); setFetchError("");
    try { setItems(await adminApi.getOwnershipTypes()); }
  ...
  function openEdit(item: OwnershipTypeConfig) { setEditTarget(item); setActionError(""); setActionSuccess(""); setShowForm(true); }
  function closeForm() { setShowForm(false); setEditTarget(null); }

  async function handleSave(payload: UpsertOwnershipTypePayload) {
    setSaving(true); setActionError(""); setActionSuccess("");
    try {
      if (editTarget) { await adminApi.updateOwnershipType(editTarget.id, payload); setActionSuccess("تم التحديث"); }
      else            { await adminApi.createOwnershipType(payload);                setActionSuccess("تم الإضافة"); }
  ...
  async function handleDelete(id: string) {
    if (!confirm("هل أنت متأكد من حذف نوع الملكية هذا؟")) return;
    setDeleting(id); setActionError(""); setActionSuccess("");
    try { await adminApi.deleteOwnershipType(id); setActionSuccess("تم الحذف"); await load(); }
    catch (err) { setActionError(normalizeError(err)); }
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "1.5rem 1rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        <h1 style={{ fontSize: "1.4rem", fontWeight: 700 }}>أنواع الملكية</h1>
      ...
      {showForm && (
        <OwnershipTypeForm initial={editTarget ?? undefined} onSave={handleSave} onCancel={closeForm} saving={saving} />
      )}
        ...
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              ...
              {items.length === 0 && (
                <tr><td colSpan={5} style={{ textAlign: "center", padding: "2rem", color: "#94a3b8" }}>لا توجد أنواع ملكية</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function OwnershipTypeForm({
  initial, onSave, onCancel, saving,
}: {
  initial?: OwnershipTypeConfig;
  onSave: (p: UpsertOwnershipTypePayload) => void;
  onCancel: () => void;
  saving: boolean;
}) {
  ...
  return (
    <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem" }}>
      <h3 style={{ margin: "0 0 1rem", fontSize: "1rem", fontWeight: 700 }}>
        {initial ? "تعديل نوع الملكية" : "إضافة نوع ملكية جديد"}
      </h3>
      <form onSubmit={(e) => { e.preventDefault(); onSave({ value: value.trim(), label: label.trim(), order: Number(order), isActive }); }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
          <div className="form-group">
            ...
            <input className="form-input" value={value} onChange={(e) => setValue(e.target.value)}
              placeholder="مثال: GreenDeed" disabled={saving} dir="ltr" />
          ...
          <div className="form-group">
            ...
            <input className="form-input" value={label} onChange={(e) => setLabel(e.target.value)}
              placeholder="مثال: طابو أخضر" disabled={saving} />
          </div>
          <div className="form-group">
          ...
          <div className="form-group" style={{ display: "flex", alignItems: "center", gap: "0.75rem", paddingTop: "1.8rem" }}>
            <input id="isActiveO" type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)}
              disabled={saving} style={{ width: 18, height: 18 }} />
            <label htmlFor="isActiveO" className="form-label" style={{ marginBottom: 0, cursor: "pointer" }}>
