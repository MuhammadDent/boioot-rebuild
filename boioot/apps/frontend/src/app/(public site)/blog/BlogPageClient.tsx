...
export default function BlogPageClient({ categories }: Props) {
  ...
  function selectCategory(slug: string | null) {
    if (slug) {
      router.replace(`/blog?category=${encodeURIComponent(slug)}`, { scroll: false });
    } else {
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.75rem" }}>
          <button
            ...
            style={{
              ...
              border: "1.5px solid " + (!activeSlug ? "var(--color-primary)" : "var(--color-border, #e5e7eb)"),
              background: !activeSlug ? "var(--color-primary)" : "transparent",
              color: !activeSlug ? "#fff" : "var(--color-text-secondary)",
