...
- **Core Features:**
    ...
    - **Plan-Based Access Control (PBAC) — Phase 1 (Frontend):**
      - `src/features/plan/types.ts` — `FeatureKey` and `LimitKey` typed unions matching backend `FeatureDefinition.Key` and `LimitDefinition.Key` exactly. Backend feature keys: `analytics_dashboard`, `video_upload`, `featured_listings`, `whatsapp_contact`, `verified_badge`, `homepage_exposure`, `project_management`. Backend limit keys: `max_active_listings`, `max_images_per_listing`, `max_agents`, `max_featured_slots`. Sentinel: `UNLIMITED = -1`.
      - `src/features/plan/plans.config.ts` — Arabic display metadata (`FEATURE_META`, `LIMIT_META`, `TIER_LABEL`, `AUDIENCE_LABEL`). Defines `FREE_TIER_DEFAULTS` for graceful degradation when subscription is unavailable.
      - `src/context/SubscriptionContext.tsx` — `SubscriptionProvider` + `usePlan()` hook. Wraps the whole app inside `AuthProvider`. Fetches `CurrentSubscriptionResponse` from `/dashboard/subscription/current` only for non-Admin/Staff authenticated users. Admin/Staff: `isAdminBypass=true` → all feature checks pass, all limits = -1. Graceful degradation: on fetch error → `subscription=null` → `canAccess` returns `false`, `getLimit` returns `FREE_TIER_DEFAULTS`. Helpers: `canAccess(feature)` (checks `subscription.features` map, then named fields), `getLimit(limit)` (checks `subscription.limits` map, then named fields), `hasPlan(planCode)`.
      - `src/components/plan/FeatureGate.tsx` — Inline component wrapper: renders children if `canAccess(feature)`, otherwise renders `UpgradePrompt` (default) or custom `fallback`. `hideIfDenied` renders nothing when denied.
      - `src/components/plan/PlanPageGuard.tsx` — Full-page route guard. Shows loading state, then renders children on access, or full-page upgrade UI (with current plan name, upgrade button, and back-to-dashboard link) on denial.
      - `src/components/plan/UpgradePrompt.tsx` — Upgrade call-to-action card. Shows feature icon, feature name, description, current plan name, and link to `/dashboard/subscription/plans`. `compact` prop renders a small inline badge.
      - `src/components/plan/LimitBadge.tsx` — `current / max unit` badge. Color-codes: green (under limit), amber (≥80%), red (at limit). Shows ∞ for unlimited.
      - `src/features/sidebar/sidebar.config.ts` — Added `feature?: FeatureKey` to `SidebarItem` and `SidebarGroup` types. Projects item has `feature: "project_management"`.
      - `src/components/dashboard/AppSidebar.tsx` — Added `usePlan()` + `canFeature(feature?)` helper alongside existing `can(permission?)`. Both RBAC + PBAC checks applied: groups and items are hidden if user lacks the required feature.
      - Provider order in `layout.tsx`: `AuthProvider > SubscriptionProvider > ContentProvider > AuthGateProvider`.
