...
- **Core Features:**
    ...
    - **Booking MVP:** Additive booking flow for DailyRent properties only. `Properties.IsBookable` gates the UI and API, `POST /api/bookings` is authenticated, blocks self-booking and non-future/invalid ranges, and stores pending booking requests in the `Bookings` table. Booking rows now persist `PricePerNight`, `TotalAmount`, `CommissionPercent`, and `CommissionAmount`; totals are calculated on create from `Property.Price × nights`, and commission uses `Booking:CommissionPercent` config with a 10% default. PostgreSQL/SQLite schema patches are idempotent and run through startup schema evolution, not manual migrations; PostgreSQL initialization runs in production and in Replit development when PostgreSQL env vars are present. Availability is exposed through `GET /api/properties/{id}/availability?startDate=&endDate=` using the same confirmed-overlap rule; booking status UI is centralized in the frontend and dashboard views show renter bookings plus owner pending/confirmed/rejected tabs.
